        ...
        <div className="grid gap-6 lg:grid-cols-[320px_minmax(0,1fr)_360px]">
          ...
          <section className="space-y-6">
            ...
            <div className="grid gap-6 xl:grid-cols-2">
              <ImagePreviewPanel
                title={L("原图", "Source Image")}
                imageUrl={sourceImageUrl}
                caption={L("AI 生成时显示 AI 原图；用户上传时显示上传原图。", "Shows the AI source image for generation or the uploaded image for uploads.")}
              />
              <ImagePreviewPanel
                title={L("主题元素提取", "Theme Element Extraction")}
                imageUrl={extractedImageUrl}
                caption={pattern?.source === "upload" ? L("已去除杂乱背景后的主体元素。", "Subject element after background cleanup.") : L("AI 生成图已按主题和文创形式生成，无需二次提取。", "AI image was generated by theme and product type, so no second extraction is needed.")}
              />
              <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                <div className="mb-3 flex items-center justify-between">
                  <h2 className="text-lg font-bold">{L("拼豆网格图纸", "Bead Grid Pattern")}</h2>
                  <span className="text-xs text-slate-500">
                    {pattern?.source === "ai" ? L("AI 图案", "AI Image") : pattern?.source === "upload" ? L("上传图", "Uploaded Image") : L("内置样例", "Built-in Sample")}
              ...
              <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                <div className="mb-3">
                  <h2 className="text-lg font-bold">{L("文创产品预览", "Cultural Product Preview")}</h2>
                  <p className="text-sm text-slate-500">
                    {L(`展示拼豆成果在${product.name}真实生活场景中的应用预览。`, `Preview how the bead work appears in a real-life ${product.name} scene.`)}
                  </p>
                </div>
                <ProductMockup
                  pattern={pattern}
            ...
            <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
              <h2 className="mb-3 text-lg font-bold">{L("导出作品", "Export Work")}</h2>
              <ExportPanel
                title={workTitle}
          ...
          <aside className="space-y-6">
            <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
              <h2 className="mb-3 text-lg font-bold">{L("材料清单", "Material List")}</h2>
              <BeadMaterialList items={beadCounts} />
            </div>
            <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
              {aiCopy ? (
                <CultureExplanation copy={aiCopy} language={language} />
              ) : (
                <div className="rounded-md border border-dashed border-slate-300 bg-slate-50 p-4 text-sm leading-6 text-slate-500">
                  {L("文化说明将由 AI 读取再创作图像后生成，并自动填入作品名称、文化来源、图案寓意和设计说明。", "Cultural notes will be generated after AI reads the recreated image, then title, source, meaning, and design notes will be filled automatically.")}
                </div>
              )}
            </div>
          </aside>
