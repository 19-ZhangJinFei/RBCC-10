...
export async function POST(req:Request){
 ...
 const apiKey=process.env.ARK_API_KEY??process.env.AI_API_KEY,baseUrl=(process.env.AI_BASE_URL??process.env.ARK_BASE_URL??"https://ark.cn-beijing.volces.com/api/v3").replace(/\/$/,""),model=process.env.AI_IMAGE_MODEL??process.env.MAIN_IMAGE_MODEL??"doubao-seedream-4-0-250828";
 if(!apiKey)return NextResponse.json({error:"服务端未配置图像生成 API Key。"}, {status:500});
